...
@section('title', 'Treasurer Notifications')
@section('page-title', 'Notifications')
@section('page-description', 'Stay updated with activities in your barangay')
...
@section('content')
            ...
            <div class="px-8 py-6 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-600">
                        Showing {{ $notifications->count() }} notifications for {{ auth()->user()->barangay }}
                    </div>
    ...
    <script>
        ...
        // Subscribe to treasurer notifications channel
        const channel = pusher.subscribe('treasurer-notifications');
        
        // Listen for new notifications
        channel.bind('new-member-registered', function(data) {
            if (data.barangay === '{{ auth()->user()->barangay }}') {
        ...
        channel.bind('benefit-request', function(data) {
            if (data.barangay === '{{ auth()->user()->barangay }}') {
                addNewNotification(data);
            }
        });

        channel.bind('contribution-made', function(data) {
            if (data.barangay === '{{ auth()->user()->barangay }}') {
                addNewNotification(data);
            }
        });
        ...
        function markAsRead(notificationId) {
            fetch(`/treasurer/notifications/${notificationId}/mark-as-read`, {
                method: 'POST',
                headers: {
