...
@section('page-description', 'Update contribution information')
...
@section('content')
        ...
        <div class="bg-gradient-to-r from-slate-800 via-blue-900 to-indigo-900 rounded-2xl p-8 text-white relative overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold mb-2">Edit Contribution</h1>
                        <p class="text-blue-100 text-lg">Update contribution information</p>
    ...
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('admin.contributions.show', $contribution) }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.contributions.update', $contribution) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select id="user_id" name="user_id" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('user_id') border-red-500 @enderror">
                        ...
                        @foreach($members as $member)
                            <option value="{{ $member->id }}" {{ (old('user_id', $contribution->user_id) == $member->id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="type" name="type" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('type') border-red-500 @enderror">
                        ...
                        <option value="weekly_savings" {{ old('type', $contribution->type) == 'weekly_savings' ? 'selected' : '' }}>Weekly Savings</option>
                        <option value="special_contribution" {{ old('type', $contribution->type) == 'special_contribution' ? 'selected' : '' }}>Special Contribution</option>
                        <option value="penalty" {{ old('type', $contribution->type) == 'penalty' ? 'selected' : '' }}>Penalty</option>
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('reference_number', $contribution->reference_number) }}"
                ...
                <div>
                    ...
                    <select id="status" name="status" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('status') border-red-500 @enderror">
                        ...
                        <option value="validated" {{ old('status', $contribution->status) == 'validated' ? 'selected' : '' }}>Validated</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" 
                              ...
                              placeholder="Optional description or notes">{{ old('description', $contribution->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="validation_notes" 
                              ...
                              rows="3"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('validation_notes') border-red-500 @enderror"
                              placeholder="Optional validation notes">{{ old('validation_notes', $contribution->validation_notes) }}</textarea>
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Current Proof of Payment</label>
                    <div class="bg-gray-50 rounded-lg p-4">
                        <img src="{{ asset('storage/' . $contribution->proof_of_payment) }}" 
                             alt="Current proof of payment" 
                             class="max-w-xs h-auto rounded-lg shadow-sm">
                        <p class="text-sm text-gray-600 mt-2">Current file</p>
                    </div>
                </div>
                @endif
                ...
                <div class="md:col-span-2">
                    <label for="proof_of_payment" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $contribution->proof_of_payment ? 'Replace Proof of Payment' : 'Proof of Payment' }}
                    </label>
            ...
            <div class="mt-8 flex items-center justify-end space-x-4">
                <a href="{{ route('admin.contributions.show', $contribution) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200">
                    Cancel
                </a>
            ...
            <div class="ml-3">
                ...
                <div class="mt-2 text-sm text-blue-700">
                    <ul class="list-disc list-inside space-y-1">
                        <li>Changing the status will affect the contribution's validation state</li>
                        <li>Validation notes help track the reason for status changes</li>
                        <li>Uploading a new proof of payment will replace the existing one</li>
