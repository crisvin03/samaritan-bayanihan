...
@section('title', 'Treasurer Management')
@section('page-title', 'Treasurer Management')
@section('page-description', 'Manage and oversee all system treasurers')
...
@section('content')
    ...
    <div class="mb-6 sm:mb-8 max-w-full overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex-1 mb-3 lg:mb-0">
                        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold mb-2 text-white">Treasurer Management</h1>
                        <p class="text-blue-100 text-sm sm:text-base mb-2 font-medium">Manage and oversee all system treasurers</p>
                        <div class="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-4">
                            ...
                            <div class="text-white/90 text-sm font-medium">
                                Total Treasurers: {{ $treasurers->count() }}
                            </div>
                        </div>
                    </div>
        ...
        <div class="mt-4 sm:mt-6 flex justify-center sm:justify-end">
            <a href="{{ route('admin.treasurers.create') }}" 
               class="group bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-3 px-6 sm:py-4 sm:px-8 rounded-lg sm:rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-xl flex items-center space-x-2 sm:space-x-3 shadow-lg w-full sm:w-auto justify-center">
                ...
                <span class="text-sm sm:text-base">Add New Treasurer</span>
            </a>
        </div>
    </div>
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input type="text" 
                           ...
                           placeholder="Search treasurers by name, email, or ID..."
                           id="search-input"
    ...
    <!-- Enhanced Treasurers Table - Responsive -->
            ...
            <table class="min-w-full divide-y divide-gray-200" style="min-width: 500px;">
                    ...
                    <tr>
                        <th class="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider" style="width: 40%;">
                            <div class="flex items-center">
                                ...
                                <span class="hidden sm:inline">Treasurer Details</span>
                                <span class="sm:hidden">Treasurer</span>
                            </div>
                        </th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($treasurers as $treasurer)
                        <tr class="group hover:bg-gradient-to-r hover:from-emerald-50 hover:to-green-50 transition-all duration-300 border-l-4 border-transparent hover:border-emerald-400">
                            <td class="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-8 w-8 sm:h-9 sm:w-9">
                                        <div class="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-gradient-to-br from-emerald-500 to-green-500 flex items-center justify-center shadow-lg group-hover:shadow-xl transition-shadow duration-300">
                                            <span class="text-xs sm:text-sm font-bold text-white">{{ substr($treasurer->name, 0, 1) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-2 sm:ml-3 min-w-0 flex-1">
                                        <div class="text-xs sm:text-sm font-bold text-gray-900 group-hover:text-emerald-900 transition-colors duration-300 truncate">{{ $treasurer->name }}</div>
                                        <div class="text-xs text-gray-600 font-medium truncate">{{ $treasurer->barangay ?? 'No barangay assigned' }}</div>
                                        <!-- Mobile: Show email and ID below name -->
                                        <div class="md:hidden mt-1">
                                            <div class="text-xs text-gray-500 truncate">{{ $treasurer->email }}</div>
                                            <div class="inline-flex items-center px-1 py-0.5 bg-green-100 text-green-800 rounded text-xs font-mono font-bold mt-1">
                                                #{{ str_pad($treasurer->id, 6, '0', STR_PAD_LEFT) }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="hidden lg:table-cell px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap">
                                <div class="text-xs font-semibold text-gray-900 truncate">{{ $treasurer->email }}</div>
                                @if($treasurer->phone_number)
                                    <div class="text-xs text-gray-600 font-medium truncate">{{ $treasurer->phone_number }}</div>
                                @endif
                            </td>
                            <td class="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold shadow-sm
                                    @if($treasurer->status === 'active') bg-gradient-to-r from-green-100 to-emerald-100 text-green-800 border border-green-200
                                    @elseif($treasurer->status === 'inactive') bg-gradient-to-r from-gray-100 to-slate-100 text-gray-800 border border-gray-200
                                    @elseif($treasurer->status === 'suspended') bg-gradient-to-r from-red-100 to-rose-100 text-red-800 border border-red-200
                                    @else bg-gradient-to-r from-gray-100 to-slate-100 text-gray-800 border border-gray-200 @endif">
                                    @if($treasurer->status === 'active')
                                        <div class="w-1.5 h-1.5 bg-green-500 rounded-full mr-1 animate-pulse"></div>
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                        </svg>
                                    @elseif($treasurer->status === 'suspended')
                                        <div class="w-1.5 h-1.5 bg-red-500 rounded-full mr-1"></div>
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M13.477 14.89A6 6 0 015.11 6.524l8.367 8.368zm1.414-1.414L6.524 5.11a6 6 0 018.367 8.367zM18 10a8 8 0 11-16 0 8 8 0 0116 0z" clip-rule="evenodd"></path>
                                        </svg>
                                    @endif
                                    <span class="hidden sm:inline">{{ ucfirst($treasurer->status) }}</span>
                                    <span class="sm:hidden">{{ ucfirst(substr($treasurer->status, 0, 3)) }}</span>
                                </span>
                            </td>
                            <td class="hidden sm:table-cell px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap">
                                <div class="text-xs font-semibold text-gray-900">{{ $treasurer->created_at->format('M d, Y') }}</div>
                                <div class="text-xs text-gray-500">{{ $treasurer->created_at->diffForHumans() }}</div>
                            </td>
                            <td class="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-right text-xs font-medium">
                                <div class="flex flex-col sm:flex-row items-end sm:items-center space-y-1 sm:space-y-0 sm:space-x-1">
                                    <a href="{{ route('admin.treasurers.show', $treasurer) }}" 
                                       class="group inline-flex items-center px-2 py-1 bg-gradient-to-r from-emerald-100 to-emerald-200 hover:from-emerald-200 hover:to-emerald-300 text-emerald-800 text-xs font-semibold rounded transition-all duration-300 shadow-sm hover:shadow-md">
                                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <a href="{{ route('admin.treasurers.edit', $treasurer) }}" 
                                    ...
                                    @if($treasurer->status === 'active')
                                        <form method="POST" action="{{ route('admin.treasurers.suspend', $treasurer) }}" class="inline">
                                            @csrf
                                            <button type="submit" 
                                                    ...
                                                    onclick="return confirm('Are you sure you want to suspend this treasurer?')">
                                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('admin.treasurers.activate', $treasurer) }}" class="inline">
                                            @csrf
                                            <button type="submit" 
                        ...
                        <tr>
                            <td colspan="6" class="px-4 sm:px-6 py-12 sm:py-16 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-2 sm:mb-3">No treasurers found</h3>
                                    <p class="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base lg:text-lg px-4">Get started by adding your first treasurer to the system.</p>
                                    <a href="{{ route('admin.treasurers.create') }}" 
                                       class="group inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-semibold rounded-lg sm:rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 text-sm sm:text-base">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
                    <div class="flex items-center text-xs sm:text-sm text-gray-700">
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="font-semibold">Showing {{ $treasurers->firstItem() }} to {{ $treasurers->lastItem() }} of {{ $treasurers->total() }} results</span>
                        </div>
                    </div>
                    <div class="flex items-center space-x-1 sm:space-x-2">
                        @if($treasurers->onFirstPage())
                        ...
                        @else
                            <a href="{{ $treasurers->previousPageUrl() }}" class="px-2 sm:px-4 py-1 sm:py-2 bg-white hover:bg-blue-50 text-gray-700 border border-gray-300 rounded-md sm:rounded-lg transition-all duration-200 font-medium shadow-sm hover:shadow-md text-xs sm:text-sm">Previous</a>
                        @endif

                        @foreach($treasurers->getUrlRange(1, $treasurers->lastPage()) as $page => $url)
                            @if($page == $treasurers->currentPage())
                        ...
                        @if($treasurers->hasMorePages())
                            <a href="{{ $treasurers->nextPageUrl() }}" class="px-2 sm:px-4 py-1 sm:py-2 bg-white hover:bg-blue-50 text-gray-700 border border-gray-300 rounded-md sm:rounded-lg transition-all duration-200 font-medium shadow-sm hover:shadow-md text-xs sm:text-sm">Next</a>
                        @else
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-6 sm:mt-8 max-w-full overflow-hidden">
        <div class="group bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            ...
            <div>
                <p class="text-xs sm:text-sm font-medium text-gray-600 mb-1">Active Treasurers</p>
                <p class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">{{ $treasurers->where('status', 'active')->count() }}</p>
                <div class="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
                    <div class="bg-gradient-to-r from-green-500 to-emerald-500 h-1.5 sm:h-2 rounded-full" style="width: {{ $treasurers->count() > 0 ? ($treasurers->where('status', 'active')->count() / $treasurers->count()) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="group bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            ...
            <div>
                <p class="text-xs sm:text-sm font-medium text-gray-600 mb-1">Inactive Treasurers</p>
                <p class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">{{ $treasurers->where('status', 'inactive')->count() }}</p>
                <div class="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
                    <div class="bg-gradient-to-r from-gray-500 to-slate-500 h-1.5 sm:h-2 rounded-full" style="width: {{ $treasurers->count() > 0 ? ($treasurers->where('status', 'inactive')->count() / $treasurers->count()) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="group bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 sm:col-span-2 lg:col-span-1">
            ...
            <div>
                <p class="text-xs sm:text-sm font-medium text-gray-600 mb-1">Suspended Treasurers</p>
                <p class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">{{ $treasurers->where('status', 'suspended')->count() }}</p>
                <div class="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
                    <div class="bg-gradient-to-r from-red-500 to-rose-500 h-1.5 sm:h-2 rounded-full" style="width: {{ $treasurers->count() > 0 ? ($treasurers->where('status', 'suspended')->count() / $treasurers->count()) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
    </div>
        ...
        if (searchInput) {
            searchInput.addEventListener('input', function(e) {
                ...
                rows.forEach(row => {
                    ...
                    const treasurerId = row.querySelector('td:nth-child(2)').textContent.toLowerCase();
                    
                    if (name.includes(searchTerm) || email.includes(searchTerm) || treasurerId.includes(searchTerm)) {
                        row.style.display = '';
                    } else {
